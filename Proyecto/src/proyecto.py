...
def unir_ocupacion_mapa(limpiar_ocupacion, cargar_geojson_2024):
    ...
    ocupacion["casos_cualificados"] = ocupacion["num_casos"].where(ocupacion["es_cualificada"], 0)
    ...
    df["porcentaje_cualificada"] = (df["ocupacion_cualificada"] / df["total_ocupacion"] * 100)
...
def grafico_renta_vs_ocupacion(unir_renta_ocupacion):
    ...
    df["casos_cualificados"] = df["num_casos"].where(df["es_cualificada"], 0)
    ...
    df_group["porcentaje_cualificada"] = (df_group["ocupacion_cualificada"] / df_group["total_ocupacion"] * 100)
