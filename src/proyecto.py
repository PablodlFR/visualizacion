...
def subir_visualizaciones_github():
    subprocess.run(["git", "add", "outputs"], cwd=BASE_DIR, check=False)
    subprocess.run(["git", "commit", "-m", "Actualización automática de visualizaciones"], cwd=BASE_DIR, check=False)
